...
import { useGetAdminAllUserQuery } from "@/utils/apiRoutes/apiEndpoint";
  ...
  if (isLoading) {
    ...
    return (
      <>
              ...
              {userDetails && userDetails.results.length > 0 ? (
                userDetails?.results.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td className="py-2 px-4 border-t capitalize">
                      {user?.active_package?.payment_status ||
        ...
        <PaginationButtons next={userDetails?.next } prev={userDetails?.previous} setPage={setPage} isLoading={isLoading}/>
      </>
    );
  }
